...
public class ClassLoaderUtil {
    ...
    /**
     * Class cache. Using soft references to not hinder GC. No upper limits, but should not be a problem, since we only
     * add some {@link SoftReference} objects to the already existing memory usage.
     */
     ...
     *
     * @param className
     * @param forName     whether to use {@link Class#forName(String, boolean, ClassLoader)} (true) or {@link
     *                    ClassLoader#loadClass(String)} (false).
     * @param classLoader
    ...
    /**
     * ClassCachePK. {@link ClassLoader} does not redefine equals and hashCode, so this may have some potential issues,
     * but usually it works fine and but best way to map a {@link Class} to its class name / {@link ClassLoader}
     * combination.
     *
    ...
    /**
     * SoftConcurrentHashMap. You can use the SoftConcurrentHashMap just like an ordinary HashMap, except that the
     * entries will disappear if we are running low on memory. Ideal if you want to build a cache.
     * <p/>
     * This implementation <b>is</b> thread-safe (just like {@link ConcurrentHashMap}). This class does <b>not</b> allow
    ...
    protected static class SoftConcurrentHashMap<K, V> extends AbstractMap<K, V> implements Serializable {
        ...
        /**
         * Returns a copy of the key/values in the map at the point of calling. However, setValue still sets the value
         * in the actual SoftHashMap.
         *
